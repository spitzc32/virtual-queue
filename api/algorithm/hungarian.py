...
try:
    ...
    raise AlgoError("NumPy is not installed.")
...
class Hungarian:
    ...
    For that numpy's resize function is used, which automatically adds
    0's to any row/column that is added
    ...
    def __init__(self, input_matrix=None, is_profit_matrix=False):
        ...
        input_matrix is assumed to be a cost matrix unless
        is_profit_matrix is True.
        """
    ...
    def calculate(self, input_matrix=None, is_profit_matrix=False):
        ...
        while len(self._results) != expected_results:
            ...
            if not zero_locations.any():
                raise AlgoError("Unable to find results. Algorithm has failed.")
    ...
    def make_cost_matrix(profit_matrix):
        ...
        ----------
            profit_matrix: np.ndarray
                matrix that we currently have to reshape it into a square matrix.
        """
    ...
    def _adjust_matrix_by_min_uncovered_num(self, result_matrix, covered_rows, covered_columns):
        """
        Subtracts m from every uncovered number and add m to every element
        ...
        ----------
            ...
            covered_rows: np.ndarray
                the covered rows (from step 1)
            covered_columns:
                the covered columns (from step 2)
        """
    ...
    def __find_matches(self, zero_locations):
        ...
        Parameters
        ----------
            zero_locations: np.ndarray
                location of zeroes in the matrix
    ...
    def __mark_rows_and_columns(marked_rows, marked_columns, row_index, column_index):
        """
        Checks if column or row is marked. If not marked then mark it.
        Parameters
        ----------
            marked_rows: np.ndarray
                already marked rows
            marked_columns: np.ndarray
                already marked columns
    ...
    def select_arbitrary_match(zero_locations):
        """
        Selects row column combination with minimum number
        of zeros in it.
        Parameters
        ----------
            zero_locations: np.ndarray
                array index of zeroes
        """
...
class CoverZeros:
    ...
    def __calculate(self):
        """
        Calculates minimum number of lines necessary to cover all
        zeros in a matrix.
        Algorithm based on: http://weber.ucsd.edu/~vcrawfor/hungar.pdf
        """
        while True:
    ...
    def __mark_new_columns_with_zeros_in_marked_rows(self):
        """
        Mark all columns not already marked which have zeros
        in marked rows.
        """
    ...
    def __mark_new_rows_with_choices_in_marked_columns(self):
        """
        Mark all rows not already marked which have choices
        in marked columns.
        """
        num_marked_rows = 0
        for index, row in enumerate(self._choices):
    ...
    def __choice_in_all_marked_columns(self):
        """
        Return Boolean True if there is a choice in all
        marked columns. Returns boolean False otherwise.
        """
    ...
    def __find_best_choice_row_and_new_column(self, choice_column_index):
        ...
        Return a random row and column if unable to find an optimal
        selection.
        """
        row_indices, = np.where(self._zero_locations[:, choice_column_index])
        for row_index in row_indices:
...
"""
Ommited due to being a sample (dummy data) to check if our algorithm 
